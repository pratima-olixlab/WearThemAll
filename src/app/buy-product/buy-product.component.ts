...
export class BuyProductComponent {
  ...
  user: UserDocument[] = []; // Get the user details
  orders: Orders[] = []; 
      ...
      if (user) {
        this.firebaseService.getUserById(user.uid).subscribe(
          ...
          (error) => {
            ...
            this.isDataLoaded = true; // Handle error state
          }
        );
      }
    });
  ...
  generatePDF() {
    ...
    const footer = () => {
      ...
      pdf.link(startX1, startY, iconSize, iconSize, { url: 'https://www.facebook.com/' });
      pdf.addImage('assets/facebook-icon.png', 'PNG', startX1, startY, iconSize, iconSize);
  
      pdf.link(startX2, startY, iconSize, iconSize, { url: 'https://twitter.com/' });
      pdf.addImage('assets/twitter-icon.png', 'PNG', startX2, startY, iconSize, iconSize);
  
      pdf.link(startX3, startY, iconSize, iconSize, { url: 'https://www.youtube.com/' });
      pdf.addImage('assets/youtube-icon.png', 'PNG', startX3, startY, iconSize, iconSize);
  
      pdf.link(startX4, startY, iconSize, iconSize, { url: 'https://www.instagram.com/' });
      pdf.addImage('assets/instagram-icon.png', 'PNG', startX4, startY, iconSize, iconSize);
    };
    ...
    this.acceptedOrders$.subscribe((orders) => {
      ...
      (pdf as any).autoTable({
        ...
        body: [
          ...
          ['Address', `${this.addresses[0].house+', '+this.addresses[0].area+', '+this.addresses[0].landmark+', '+
            this.addresses[0].city+', '+this.addresses[0].state+', '+this.addresses[0].country+' - '+this.addresses[0].pincode}`]
        ],
        columnStyles: {
      ...
      (pdf as any).autoTable({
        head: [['Order', 'Status', 'Total Amount in Rs', 'Product', 'Name', 'Price']],
        body: ordersData,
